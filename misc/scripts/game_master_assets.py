...
def _hex_to_rgba(value: str | tuple[int, int, int, int]) -> tuple[int, int, int, int]:
...
def _write_text(path: Path, text: str) -> None:
    """Write text with LF endings (``Path.write_text`` has no ``newline`` knob on 3.9)."""
    path.parent.mkdir(parents=True, exist_ok=True)
    with open(path, "w", encoding="utf-8", newline="\n") as f:
        f.write(text)


def write_ico(images: list[Any], path: Path) -> Path:
...
def write_icns(images: dict[str, Any], path: Path) -> Path:
...
def generate(threshold: int) -> int:
    ...
    app_icon_size = ENGINE_ASSETS["main/app_icon.png"]
    assert isinstance(app_icon_size, int)
    splash_size = ENGINE_ASSETS["main/splash.png"]
    assert isinstance(splash_size, int)
    splash_editor_size = ENGINE_ASSETS["main/splash_editor.png"]
    assert isinstance(splash_editor_size, tuple)
    emit(square(badge, app_icon_size, margin=0.06), "main/app_icon.png")
    emit(square(badge, splash_size, margin=0.10), "main/splash.png")
    emit(wide_logo(badge, wordmark, splash_editor_size), "main/splash_editor.png")
    ...
    _write_text(ROOT / "misc/brand/linux/hicolor/scalable/apps/game-master.svg", svg)
...
def preview() -> int:
    ...
    def label(text: str, x: float, y: float, fill: tuple[int, int, int, int] = (230, 230, 235, 255)) -> None:
        if font is not None:
    ...
    def layer(source_image: Any, tint: Any = None) -> Any:
