...
def _hex_to_rgba(value: object) -> tuple[int, int, int, int]:
...
def text_mask(font_path: Path, text: str, size: float):
...
def credit_splash(badge, size: int = 1024):
    ...
    draw.rounded_rectangle(
        (rx0, ry0, rx0 + ribbon_w, ry0 + ribbon_h), radius=30, fill=COLOR_INK, outline=COLOR_GOLD, width=5
    )

    def put_line(mask, fill, x: int, y: int) -> None:
    ...
    if line_mask.width > max_w:
        ...
        line_mask = text_mask(FONT_DISPLAY, AUTHOR_LINE, line_size)
    put_line(line_mask, COLOR_WHITE, (size - line_mask.width) // 2, ry0 + ribbon_h - line_mask.height - 46)
    return canvas
...
def write_icns(images: dict[Any, Any], path: Path) -> Path:
...
def generate(threshold: int) -> int:
    ...
    if _fonts_available():
        ...
        print(
            "warning: the display font in misc/scripts/fonts/ is missing; "
    ...
    default_icon.write_text(default_project_icon_svg(), encoding="utf-8")
    ...
    (ROOT / "misc/brand/linux/hicolor/scalable/apps/game-master.svg").write_text(svg, encoding="utf-8")
...
def preview() -> int:
    ...
    def layer(source_image, tint=None):
...
def main() -> int:
    ...
    if args.svg_only:
        ...
        path.write_text(default_project_icon_svg(), encoding="utf-8")
        print(f"wrote {path.relative_to(ROOT)}")
        return 0
    if args.preview:
        return preview()
